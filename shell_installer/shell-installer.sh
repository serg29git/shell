#!/bin/bash
# shell-installer.sh - installer for the 'shell' command
# Automatically detects directories, uses sudo-first fallback
...
check_existing() {
    ...
    elif command -v shell &>/dev/null; then
        command -v shell
    else
        echo ""
    fi
...
main() {
    ...
    echo "found dir for shell installation: $BIN_DIR"
    ...
    if [ -n "$EXISTING_BIN" ]; then
        echo "another shell was found: $EXISTING_BIN"
    fi
    
    # Ask for installation
    echo
    read -p "install shell? [Y/n]: " install_choice
    ...
    if install_with_sudo_fallback "$SCRIPT_PATH" "${BIN_DIR}/${SCRIPT_NAME}" 755; then
        info "installed shell into $BIN_DIR"
    else
        error "could not install shell to $BIN_DIR (even with sudo). Aborting."
    fi
    ...
    echo "  1  test shell"
    echo "  9  delete shell (for some reason)"
    echo "  0  exit"
    echo
    read -p ": " choice
    case "$choice" in
        1)
            if command -v shell &>/dev/null; then
                ...
                warn "shell not in PATH. Try running ${BIN_DIR}/${SCRIPT_NAME}"
            fi
            ;;
        9)
